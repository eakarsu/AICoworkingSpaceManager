...
/**
 * Gap/CFS Feature Page: Limited payment integration — only Stripe stub.
 * Slug: limited-payment-integration-only-stripe-stub
 * Backend: /api/gap-limited-payment-integration-only-stripe-stub
 */
export default function LimitedPaymentIntegrationOnlyStripeStubPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const res = await fetch('/api/gap-limited-payment-integration-only-stripe-stub', {
        method: 'POST',
        headers: {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 900, margin: '0 auto', color: '#e5e7eb' }}>
      <h2 style={{ marginBottom: 8 }}>Limited payment integration — only Stripe stub.</h2>
      <p style={{ opacity: 0.75, marginBottom: 16 }}>Limited payment integration — only Stripe stub.</p>
