...
const FEATURE_KEY = process.env.FEATURE_DYNAMIC_PRICING_KEY;
const SYSTEM_PROMPT = `You are an expert assistant specialized in: Dynamic pricing & revenue optimization.
...
async function callLLM(userPayload) {
  ...
  const response = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AICoworkingSpaceManager - dynamic-pricing',
    },
    body: JSON.stringify({
...
router.post('/dynamic-pricing', auth, async (req, res) => {
  try {
    ...
    if (!FEATURE_KEY) res.set('X-Feature-Credentials-Missing', 'FEATURE_DYNAMIC_PRICING_KEY');
    const ai = await callLLM({ feature: 'dynamic-pricing', goal: 'Dynamic pricing & revenue optimization', input: payload });
    try {
      if (pool && pool.query) {
        await pool.query(
          ...
          ['dynamic-pricing', JSON.stringify(payload), JSON.stringify(ai.result), ai.model, req.user?.id || null]
        );
      }
    } catch (persistErr) {
      console.warn('[dynamic-pricing] persistence skipped:', persistErr.message);
    }
    return res.json({
      ok: true,
...
router.get('/dynamic-pricing/health', (req, res) => {
  res.json({
    feature: 'dynamic-pricing',
    endpoint: '/api/ai/dynamic-pricing',
